...
class TrainModel(object):
    ...
    def __init__(self, content_filename, style_filename, quality='medium', style_weight=750000, num_steps=500):
        ...
        self.loader = T.Compose([T.Resize((self.imsize, self.imsize)),
                                 T.ToTensor()])
        self.H = self.image_loader(content_filename)[1][0]
        self.W = self.image_loader(content_filename)[1][1]

        self.style_img = self.image_loader(style_filename)[0]
        self.content_img = self.image_loader(content_filename)[0]
        ...
        self.cnn = torch.load('vgg_for_NST.pth')
    ...
    def image_loader(self, fname):
        size = self.imsize
        img = PIL.Image.open(fname).convert('RGB')
        W, H = img.size
        img = self.loader(img).unsqueeze(0)
        return img.to(self.device, torch.float), (H, W)

    def run_style_transfer(self, out_name):

        model, style_losses, content_losses = self.get_style_model_and_losses(self.cnn,
                                                                              self.normalization_mean,
                                                                              self.normalization_std, self.style_img,
                                                                              self.content_img)
        ...
        while run[0] <= self.num_steps:
            ...
            def closure():
                ...
                run[0] += 1
                if run[0] % 50 == 0:
                    print("run {}:".format(run), file=sys.stderr)
                    print('Style Loss : {:4f} Content Loss: {:4f}'.format(
                        style_score.item(), content_score.item()), file=sys.stderr)
        ...
        save_image(T.Resize((self.H, self.W))(self.input_img.squeeze(0)), out_name)
